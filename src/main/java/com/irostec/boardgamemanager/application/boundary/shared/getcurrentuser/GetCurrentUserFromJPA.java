...
import static com.irostec.boardgamemanager.common.utility.Functions.wrapWithFunctionalErrorHandling;

@Component
class GetCurrentUserFromJPA implements GetCurrentUser {

    private final Logger logger = LogManager.getLogger(GetCurrentUserFromJPA.class);

    private final BoardGameUserRepository userRepository;

    GetCurrentUserFromJPA(BoardGameUserRepository userRepository) {
    ...
    private Either<GetCurrentUserError, Long> getUserId(String username) {
        ...
        Either<GetCurrentUserError, Optional<BoardGameUser>> optionalUserContainer = wrapWithFunctionalErrorHandling(
            () -> {
